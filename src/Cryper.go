...
func draw() {
	giu.SingleWindow().Flags(524351).Layout(
		...
		giu.Separator(),
		giu.Style().SetDisabled((len(allFiles) == 0 && len(onlyFiles) == 0) || scanning).To(
			giu.Label("Enter Password for encrypting:"),
			giu.Row(
				...
				giu.Tooltip("Clear the entered password"),
...
func main() {
	...
	window = giu.NewMasterWindow("Cryper", 318, 507, giu.MasterWindowFlagsNotResizable)
